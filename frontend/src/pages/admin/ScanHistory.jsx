...
export const AdminScanHistory = () => {
  ...
  const fetchHistory = async () => {
    setLoading(true);
    try {
      const data = await api.admin.getScanHistory();
      setHistory(data);
    } catch (err) {
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute("download", `all_scan_history_${new Date().toISOString().split('T')[0]}.csv`);
    ...
    <div className="space-y-6 animate-fade-in max-w-5xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-600 mt-2">Log and audit records of all verified gate passes by guards</p>
        </div>
          ...
          <div className="py-20 text-center">
            ...
            <p className="text-slate-400 text-sm mt-1">
              {searchQuery || selectedYearFilter !== "all"
                ...
                : "No scanned gate logs exist in the system yet."}
            </p>
          </div>
        ) : (

...
export const AdminNotices = () => {
  ...
  const fetchNotices = async () => {
    try { setNotices(await api.admin.getNotices()); }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingNotice) {
        const updated = await api.admin.updateNotice(editingNotice.id, payload);
        setNotices((prev) => prev.map((n) => (n.id === editingNotice.id ? updated : n)));
        success("Notice updated successfully");
      } else {
        const created = await api.admin.createNotice(payload);
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.admin.deleteNotice(deleteTarget.id);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-600 mt-1">Create, edit and manage hostel announcements</p>
        </div>
